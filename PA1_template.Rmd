...
The mean number of steps was `r MeanSteps` steps per day while the median number of steps was only slightly lower at `r MedSteps` steps.  Note that these values are so similar that they are not distinguishable on the histogram. 
...
sumNA<-sum(stepsNA)
```
First, note that there are `r sumNA` missing values.

However, missing values are not distributed randomly throughout the data set.  
```{r missing.days, echo=T}
activityNA<-activity[stepsNA,]
t.NAdate<-table(activityNA$date)
t.NAdateOnly<-t.NAdate[t.NAdate>0]
NAdates<-length(t.NAdateOnly)
```
Instead, there are just `r NAdates` days with missing values. The dates with missing values are indicated in the figure below:
```{r plot_missing.days, echo=T}
StepsDay<-tapply(activity$steps, activity$date, sum)
